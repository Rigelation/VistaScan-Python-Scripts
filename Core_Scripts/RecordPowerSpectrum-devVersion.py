...
DISTANCE_LIFT = 0;        #microns
CHANNELID_PIFM = 17;
SIGNAL_REQUIRED = .0002;        #volts
LASER_INDEX = 0

SPECTRUM_SECONDS = 30;
SPECTRUM_REPEATCOUNT = 2;

VistaScan.Connect();

##To do
#Find LASER_INDEX value 
#Set spectrum to full range if not already
...
#    VistaScan.MoveHeadToPosition(positionHead + DISTANCE_LIFT);
    ...
    VistaScan.SetLaserPowerstate(LASER_INDEX, 0) #Turn QCL off [improve]
    print("Auto sweep");
...
#!    must add, or make this automatic for the laser
    ...
    VistaScan.SetLaserPowerstate(LASER_INDEX, 1) #Turn QCL on [improve]
    time.sleep(0.5)
